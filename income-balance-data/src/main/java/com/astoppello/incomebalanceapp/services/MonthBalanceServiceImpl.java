...
public class MonthBalanceServiceImpl implements MonthBalanceService {
    ...
    public MonthBalanceDTO saveMonthBalance(
            Long monthBalanceId, MonthBalanceDTO monthBalanceDTO) {
        MonthBalance monthBalance = monthBalanceMapper.monthBalanceDtoToMonthBalance(monthBalanceDTO);
        yearBalanceRepository.findById(monthBalanceDTO.getYearBalanceId()).ifPresent(monthBalance::setYearBalance);
        return createAndReturnDto(monthBalance);
    }
    ...
    public MonthBalanceDTO updateMonthBalance(
       ...
       return monthBalanceRepository.findById(monthBalanceId).map(monthBalance -> {
            ...
            if (monthBalanceDTO.getYearBalanceId() != null) {
                yearBalanceRepository.findById(monthBalanceDTO.getYearBalanceId())
                                     .ifPresent(monthBalance::setYearBalance);
            }
            //TODO: Check this better
            if (CollectionUtils.isNotEmpty(monthBalanceDTO.getBankBalances())) {
                monthBalance.setBankBalanceList(monthBalanceDTO.getBankBalances().stream()
