...
public class MonthBalanceControllerTest {
    ...
    void testFindMonthBalanceById() throws Exception {
        ...
        mockMvc.perform(get(MonthBalanceController.BASE_URL + "/1").contentType(MediaType.APPLICATION_JSON))
               .andExpect(status().isOk())
               .andExpect(jsonPath("$.month", equalTo(MONTH)))
               .andExpect(jsonPath("$.id", equalTo(1)));
    ...
    void testCreateNewMonthBalance() throws Exception {
        when(monthBalanceService.createNewMonthBalance(any(MonthBalanceDTO.class))).thenReturn(monthBalanceDTO);
        mockMvc
                .perform(
